...
class ScalePainter extends IndicatorPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var index = 0; index < count; index++) {
      ...
      var activeFillColor = effect.dotColor;
      var scale = 0.0;
      if (index == current) {
        scale = (effect.scale) - (activeScale * dotOffset);
        color = Color.lerp(effect.activeDotColor, effect.dotColor, dotOffset);
        activeFillColor = Color.lerp(
            effect.activeDotFillColor ?? effect.activeDotColor,
            effect.dotColor,
            dotOffset);
      ...
      if (effect.activeDotFillColor != null) {
        canvas.drawRRect(
            _calcBounds(size.height, index), dotPaint..color = activeFillColor);
      }
      canvas.drawRRect(
